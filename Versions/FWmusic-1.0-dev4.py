...
def download_click(page, name, x_1, y_1, x_2, y_2):  # v.1.1.0
...
def steal_music_from_yeyulingfeng(name_list, author):  # ver:1.0.3+1.1.0
    with sync_playwright() as p:
        ...
        for name in name_list:
            ...
            with page_new.expect_download() as download_info:
                # pyautogui.click(x_1, y_1)
                # time.sleep(1)
                # pyautogui.moveTo(x_2, y_2)
                # pyautogui.click()
...
#steal_music_from_yeyulingfeng(name_list, author)
steal_music_from_yeyulingfeng(['花间游'], '忘乡')
...
with sync_playwright() as p:
    browser = p.chromium.launch(channel="chrome", headless=False)
    page = browser.new_page()
    page.goto('https://m801.music.126.net/20240908185715/99a786c022a794238e065fb062637d14/jdymusic/obj/wo3DlMOGwrbDjj7DisKw/28482000958/78e4/3aa4/6969/19f78c15e926ff8b6e7d46d0b2d9f5b8.mp3')
    with page.expect_download() as download_info:
        #download_btn = page.get_by_text("下载").first
        #download_btn.click()
        time.sleep(5)
        download = download_info.value
        download.save_as("test.mp3")
    browser.close()
'''